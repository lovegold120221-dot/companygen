...
export const InputArea: React.FC<InputAreaProps> = ({ onGenerate, isGenerating, disabled = false }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto perspective-1000 flex flex-col gap-4">
      {/* Mode Toggle */}
      <div className="flex bg-zinc-900/50 p-1 rounded-xl mx-auto border border-zinc-800 w-full max-w-xs transition-opacity duration-300">
          <button 
            type="button"
            className={`flex-1 text-sm font-medium py-2 rounded-lg transition-all ${mode === 'upload' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
          ...
          <button 
            type="button"
            className={`flex-1 text-sm font-medium py-2 rounded-lg transition-all ${mode === 'generate' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
      ...
      >
        ...
        >
            ...
            <div className="relative z-10 flex flex-col items-center justify-center w-full max-w-2xl px-6 md:px-8 space-y-6">
                
                {isGenerating ? (
                  <div className="flex flex-col items-center space-y-4 pt-4">
                     <div className="w-16 h-16 rounded-2xl bg-zinc-800 border border-zinc-700 shadow-xl flex items-center justify-center animate-pulse">
                         <SparklesIcon className="w-8 h-8 text-blue-400 animate-pulse" />
                     </div>
                     <p className="text-zinc-400 font-light tracking-wide">Generating image...</p>
                  </div>
                ) : (
                  <>
                    <h3 className="text-xl sm:text-2xl md:text-3xl text-zinc-100 font-bold tracking-tighter text-center">
                    ...
                    <div className="w-full flex flex-col space-y-4">
                      <textarea 
                        ...
                        className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-4 text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500/50 resize-none h-24 placeholder:text-zinc-600"
                        disabled={isGenerating || disabled}
                      />
                      <button 
                        type="button"
                        ...
                        className="w-full py-3 px-4 bg-zinc-100 text-zinc-900 font-medium rounded-lg hover:bg-white disabled:bg-zinc-800 disabled:text-zinc-600 transition-colors flex items-center justify-center gap-2"
                      >
